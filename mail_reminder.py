...
class NlpEnglish:
    ...
    def check_time(self):
        ...
        if any(word in time_segment for word in key):
            in_time = datetime.datetime.strptime(time_segment, "%I:%M %p")
...
def trans_to_eng(text):
    ...
    if sample.detect_language() != 'en':
        return sample.translate(to='en')
    elif sample.detect_language() == 'en':
        return sample
...
def send_email():
    ...
    message.set_content('30 min to go fot the event')
...
if __name__ == "__main__":
    input_text = 'deadline for submission of the form is today by 4:00'
